import os
from pathlib import Path
import subprocess
import tempfile
import unittest

ROOT = Path(__file__).parents[1]


class UpdateContractTests(unittest.TestCase):
    def test_local_update_is_repeatable_without_git_or_pr_writes(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            binary = root / "bin"
            binary.mkdir()
            target = root / "ysh.rb"
            initial = 'class Ysh < Formula\n  url "https://github.com/azohra/yaml.sh/releases/download/v1.0.0/ysh"\n  sha256 "' + '0' * 64 + '"\nend\n'
            target.write_text(initial)
            (binary / "gh").write_text('''#!/bin/sh
set -eu
case "$1 $2" in
  "release view") printf true ;;
  "release download")
    while [ "$1" != --dir ]; do shift; done
    cd "$2"
    printf 'artifact\\n' > ysh
    shasum -a 256 ysh > ysh.sha256
    ;;
  *) echo "unexpected GitHub write: $*" >&2; exit 99 ;;
esac
''')
            (binary / "gh").chmod(0o755)
            environment = {**os.environ, "PATH": f"{binary}:{os.environ['PATH']}"}
            def update():
                return subprocess.run(["sh", str(ROOT / "scripts/update-ysh.sh"), "1.1.0"], cwd=root, env=environment, capture_output=True, text=True)
            first = update()
            self.assertEqual(first.returncode, 0, first.stderr)
            updated = target.read_text()
            self.assertIn("1.1.0", updated)
            second = update()
            self.assertEqual(second.returncode, 0, second.stderr)
            self.assertEqual(target.read_text(), updated)
            self.assertFalse((root / ".git").exists())
